...
{
	$inFilter = $GLOBALS[$arParams["FILTER_NAME"]];
...
if (Loader::includeModule("inetris.nocode")) {
    $dto = new \Inetris\Nocode\DTO\Settings($inFilter, $arParams["IBLOCK_ID"], $arParams["PARENT_SECTION_CODE1"], $arParams["CACHE_TIME"], 10000, false);
    \Bitrix\Main\DI\ServiceLocator::getInstance()->addInstance("inetris.nocode.settings", $dto);

    $related_news = \Bitrix\Main\DI\ServiceLocator::getInstance()->get('inetris.nocode.nocode');
...
if($this->startResultCache(false, array(($arParams["CACHE_GROUPS"]==="N"? false: $USER->GetGroups()), $bUSER_HAVE_ACCESS, $arNavigation, $arrFilter, $pagerParameters)))
{
	...
	$rsElement = CIBlockElement::GetList(array("id" => $arrFilter['ID']), array_merge($arFilter , $arrFilter), false, $arNavParams, $shortSelect);
